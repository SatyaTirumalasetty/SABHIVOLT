...
export function EnquiryForm({ onOpenPrivacy }) {
  const [form, setForm] = useState({
    name: "", phone: "", email: "",
    interest: INTEREST_OPTIONS[0], message: "",
  ...
  const submit = async () => {
    if (form.website.trim() !== "") { setState("done"); return; } // bot trap
    if (!form.name.trim() || (!form.phone.trim() && !form.email.trim())) {
      setState("invalid");
      return;
    }
    const now = Date.now();
    if (now - lastSubmitRef.current < MIN_SUBMIT_GAP_MS) { setState("slow"); return; }
    setState("sending");
    ...
    <div className="sv-form">
      ...
      {state === "done" ? (
        <div className="sv-success">
          ...
          <p>Thanks{form.name ? `, ${form.name.split(" ")[0]}` : ""}. We'll get back to you within two working days.</p>
        </div>
      ) : (
        <>
          ...
          <div className="sv-form-row">
            ...
            <div className="sv-field">
              ...
              <input id="enq-phone" value={form.phone} onChange={set("phone")} autoComplete="tel" inputMode="tel" />
            </div>
          </div>
          <div className="sv-field">
            <label htmlFor="enq-email">Email</label>
            <input id="enq-email" value={form.email} onChange={set("email")} autoComplete="email" inputMode="email" />
          </div>
          <div className="sv-field">
          ...
          <div className="sv-field">
            ...
            <textarea id="enq-message" value={form.message} onChange={set("message")} placeholder="Tell us about your land, fleet or project…" />
          </div>
          ...
          {state === "invalid" && <p className="sv-form-note err">Add your name and at least a phone number or email.</p>}
          {state === "err" && (
          ...
          {state === "slow" && <p className="sv-form-note err">Please wait a moment before sending another enquiry.</p>}
          <p className="sv-consent">
            By sending this enquiry you agree that SABHIVOLT may use these details to respond to you.
            See our <button onClick={onOpenPrivacy}>privacy &amp; terms</button>.
          </p>
        </>
      )}
    </div>
...
export function LoginModal({ onSuccess, onCancel }) {
  ...
  const tryLogin = async () => {
    ...
    setBusy(false);
    if (error) {
      setErr(backendConfigured ? "Sign-in failed. Check your email and password." : "Backend not configured — set Supabase env vars first.");
      setPassword("");
    } else {
      onSuccess();
    }
  ...
  return (
    <div className="sv-overlay" role="dialog" aria-modal="true" aria-label="Admin sign in">
      <div className="sv-modal sv-pin-box" ref={ref} style={{ width: "min(400px, 100%)" }}>
        <h2>Admin sign in</h2>
        ...
        <div className="sv-pin-row">
          <button className="sv-btn sv-btn-solid" onClick={tryLogin} disabled={busy}>
            {busy ? "Signing in…" : "Sign in"}
          </button>
          <button className="sv-btn sv-btn-ghost" onClick={onCancel}>Cancel</button>
        </div>
      </div>
    </div>
  );
